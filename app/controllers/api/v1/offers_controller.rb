class Api::V1::OffersController < ApplicationController
  ...
  def get_index
    ...
    if params[:city]
      commerces = Commerce.where(published: true, city: params[:city]).map{|x| [x.id, 'commerce', (x.name+' '+x.description), x.name, x.description, x.name, nil, nil ]}
    elsif
      commerces = Commerce.where(published: true).map{|x| [x.id, 'commerce', (x.name+' '+x.description), x.name, x.description, x.name, nil, nil  ]}
    end
