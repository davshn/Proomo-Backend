class Api::V1::UsersController < ApplicationController
  ...
  def get_points
    user = User.find(params[:id])
    total_points = (user.purchaces.where(return_code:"OK", validate_sale: true).select{|x| x.total >= 10000}.map{|x| x.total}.inject(0){|sum,x| sum + x } / 10000)
    spent_points = user.purchaces.where(return_code:"OK", user_payment_method: 'points').select{|x| x.offer_id != nil}.map{|x| Offer.find(x.offer_id).redeem_points}.inject(0){|sum,x| sum + x }
