class Api::V1::UsersController < ApplicationController
  ...
  def get_points
    user = User.find(params[:id])
    total_points = (user.purchaces.where(return_code:"OK").map{|x| x.total}.inject(0){|sum,x| sum + x } / 10000)
    spent_points = user.purchaces.where(return_code:"OK").select{|x| x.offer_id != nil}.map{|x| Offer.find(x.offer_id).points}.inject(0){|sum,x| sum + x }
